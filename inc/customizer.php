...
function theme_customize_register($wp_customize) {
    ...
    for ($i = 1; $i <= 5; $i++) {
        ...
        $wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, "color_$i", array(
            'label' => sprintf(__('Color %s', 'ntfs'), $i),
            'section' => 'colors',
        )));
    }
    ...
    $wp_customize->add_control('typography_scheme', array(
        'label' => __('Typography Scheme', 'ntfs'),
        'section' => 'typography',
        'type' => 'select',
        'choices' => array(),
    ));
    ...
    $wp_customize->add_control('typography_heading_weight', array(
        'label' => __('Heading Weight', 'ntfs'),
        'section' => 'typography',
        'type' => 'select',
        'choices' => array(
            '100' => __('100 Thin', 'ntfs'),
            '300' => __('300 Light', 'ntfs'),
            '400' => __('400 Regular', 'ntfs'),
            '500' => __('500 Medium', 'ntfs'),
            '700' => __('700 Bold', 'ntfs'),
            '900' => __('900 Black', 'ntfs'),
        ),
    ));
    ...
    $wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, 'logo_width', array(
        'type' => 'number',
        'label' => __('Logo max width (px)', 'ntfs'),
    ...
    $wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, 'logo_height', array(
        'type' => 'number',
        'label' => __('Logo max height (px)', 'ntfs'),
        'section' => 'title_tagline',
        'priority' => 9,
    )));
    ...
    $wp_customize->add_control(new WP_Customize_Cropped_Image_Control($wp_customize, 'custom_favicon', array(
        'label' => __('Favicon', 'ntfs'),
        'section' => 'title_tagline',
        'priority' => 8,
        'height' => '64',
        'width' => '64',
        'button_labels' => array(
            'select' => __('Select favicon', 'ntfs'),
            'change' => __('Change favicon', 'ntfs'),
            'remove' => __('Remove', 'ntfs'),
            'default' => __('Default', 'ntfs'),
            'placeholder' => __('No favicon selected', 'ntfs'),
            'frame_title' => __('Select favicon', 'ntfs'),
            'frame_button' => __('Choose favicon', 'ntfs'),
        ),
    )));
    ...
    $wp_customize->add_control('menu_trim_title', array(
        ...
        'label' => __('Trim long menu items', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('menu_trim_len', array(
        'type' => 'number',
        'section' => 'menu_options',
        'label' => __('Limit each item to [N] characters', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('submenu_trim_len', array(
        'type' => 'number',
        'section' => 'menu_options',
        'label' => __('Limit each subitem to [N] characters', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('menu_use_tag_filter', array(
        'type' => 'checkbox',
        'section' => 'menu_options',
        'label' => __('Apply menu item tag filter', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('menu_allowed_tags', array(
        'type' => 'text',
        'section' => 'menu_options',
        'label' => __('Allowed menu item tags', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('use_default_menu', array(
        'type' => 'checkbox',
        'section' => 'menu_options',
        'label' => __('Use not stylized menu', 'ntfs'),
        'description' => __('Used standart wp_nav_menu when option is enabled (need for some third-party plugins).', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('excerpt_auto', array(
        ...
        'label' => __('Use auto excerpts', 'ntfs'),
        'description' => __('Generate post excerpts automatically (When neither more-tag nor post excerpt is used)', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('excerpt_words', array(
        'type' => 'number',
        'section' => 'excerpt_options',
        'label' => __('Excerpt length (words)', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('excerpt_min_remainder', array(
        'type' => 'number',
        'section' => 'excerpt_options',
        'label' => __('Excerpt balance (words)', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('excerpt_strip_shortcodes', array(
        'type' => 'checkbox',
        'section' => 'excerpt_options',
        'label' => __('Remove shortcodes from excerpt', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('excerpt_use_tag_filter', array(
        'type' => 'checkbox',
        'section' => 'excerpt_options',
        'label' => __('Apply excerpt tag filter', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('excerpt_allowed_tags', array(
        'type' => 'text',
        'section' => 'excerpt_options',
        'label' => __('Allowed excerpt tags', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('morelink_template', array(
        'type' => 'text',
        'section' => 'excerpt_options',
        'label' => __('More Link Template', 'ntfs'),
        'description' => sprintf(__('ShortTags: %s', 'ntfs'), '[url], [text]'),
    ));
    ...
    $wp_customize->add_control('seo_og', array(
        'type' => 'checkbox',
        'section' => 'seo_options',
        'label' => __('Include Open Graph meta tags', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('seo_ld', array(
        'type' => 'checkbox',
        'section' => 'seo_options',
        'label' => __('Include schema.org JSON-LD syntax markup', 'ntfs'),
    ));
                ...
                if (isset($op['options'])) {
                    foreach ($op['options'] as $key => $option) {
                        if (strpos($key, $templateKey . 'Template') === 0) {
                            ...
                            $templatesOptions[$templateKey][$key] = __($option, 'ntfs');
                        }
                    }
                }
            }
    ...
    if (file_exists(get_template_directory() . '/shop/products.json')) {
        ...
        if (isset ($data['products']) && $data['products']) {
            $wp_customize->add_section('products_options', array(
                'capability' => 'edit_theme_options',
                'title' => __('E-Commerce Templates', 'ntfs'),
                'priority' => 200,
            ));
            ...
            $wp_customize->add_control('links_title', array(
                ...
                'label' => __('Theme With E-Commerce Templates', 'ntfs'),
...
</br></br>Please copy the following links and paste them into your CMS Menu Item.', 'ntfs'),
            ));
            ...
            $wp_customize->add_control('products_info', array(
                'type' => 'text',
                'section' => 'products_options',
                'label' => __('Product List Page Template:', 'ntfs'),
            ...
            $wp_customize->add_control('product_info', array(
                'type' => 'text',
                'section' => 'products_options',
                'label' => __('Product Details Page Template:', 'ntfs'),
    ...
    $wp_customize->add_control('include_jquery', array(
        ...
        'label' => __('Use jQuery from theme', 'ntfs'),
    ));
    ...
    $wp_customize->add_section('sidebar_options', array(
        'capability' => 'edit_theme_options',
        'title' => __('Sidebars', 'ntfs'),
        'priority' => 100,
    ));

    $layouts_choices = array(
        '' => __('Default', 'ntfs'),
        'left-sidebar' => __('Left', 'ntfs'),
        'right-sidebar' => __('Right', 'ntfs'),
        'none' => __('None', 'ntfs'),
    );
    ...
    $wp_customize->add_control('sidebars_layout_blog', array(
        ...
        'label' => __('Blog Template', 'ntfs'),
        'description' => __('Templates for displaying blogs, archives, search results', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('sidebars_layout_post', array(
        ...
        'choices' => $layouts_choices,
        'section' => 'sidebar_options',
        'label' => __('Post Template', 'ntfs'),
        'description' => __('Templates for displaying single posts', 'ntfs'),
    ));
    ...
    $wp_customize->add_control('sidebars_layout_default', array(
        ...
        'choices' => $layouts_choices,
        'section' => 'sidebar_options',
        'label' => __('Default Template', 'ntfs'),
        'description' => __('Templates for displaying single pages and everything else', 'ntfs'),
    ));
}
add_action('customize_register', 'theme_customize_register', 11);

function theme_print_sidebars_blog_caption() {
    printf('<h3>%s</h3>', __('Display sidebars on:', 'ntfs'));
}
