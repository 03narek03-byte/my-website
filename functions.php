...
if (!function_exists('theme_setup')) {
    ...
    function theme_setup() {
        ...
        $GLOBALS['npThemeVersion'] = '7.11.17';
    }
...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'ntfs'),
        'collapse' => __('collapse child menu', 'ntfs'),
    ));
}
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'ntfs'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'ntfs'),
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'ntfs');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'ntfs'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'ntfs'));
}
...
function getProductButtonHtml ($button_html, $product, $clickTypeProductbutton = 'add-to-cart', $contentProductbutton = '') {
    ...
    $buttonText = sprintf(__('Add to cart', 'ntfs'));
    if ($contentProductbutton !== '') {
        $buttonText =  sprintf(__($contentProductbutton, 'ntfs'));
    }
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'ntfs'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'ntfs')
    ));
    extract($args);
...
function blog_sorting() {
    $blog_sorting_options = array(
        'name_asc'  => __('Name, A to Z', 'ntfs'),
        'name_desc' => __('Name, Z to A', 'ntfs'),
        'date_asc'  => __('Date, old to new', 'ntfs'),
        'date_desc' => __('Date, new to old', 'ntfs'),
    );
