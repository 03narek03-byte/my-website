...
class Npwizard {
    ...
    public function enqueue_scripts() {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'verify_text'    => esc_html('verifying', 'ntfs'),
    ...
    public function menu_page() {
        add_theme_page(esc_html__('Theme Wizard', 'ntfs'), esc_html__('Theme Wizard', 'ntfs'), 'manage_options', $this->page_slug, array($this, 'wizard_page'));
    }
    ...
    public function wizard_page() {
        ...
        <div class="wrap npwizard-wrap-perent">
            ...
            if (defined('APP_PLUGIN_VERSION') && isset($GLOBALS['npThemeVersion']) && (float)APP_PLUGIN_VERSION > (float)$GLOBALS['npThemeVersion']) {
                ...
                echo sprintf('<div class="npwizard-warning"><p>%s</p></div>', 'The active theme has a version lower than the plugin version. Please update the theme too.', 'ntfs');
            }
            ?>
        </div><!-- .npwizard-wrap -->
    ...
    public function get_steps() {
        $steps = array(
            'done' => array(
                'id'			=> 'done',
                'title'			=> __('Your website is ready!', 'ntfs'),
                'icon'			=> 'yes',
                'view'			=> 'get_step_done',
                'callback'		=> ''
            )
        ...
        $import_content_step = array(
            'content' => array(
                'id'			=> 'content',
                'title'			=> __('Import Content', 'ntfs'),
                'icon'			=> 'welcome-content-menus',
                'view'			=> 'get_step_content',
                'callback'		=> 'import_content',
                'callback2'		=> 'replace_content',
                'button_text'	=> __('Import Content', 'ntfs'),
                'button2_text'	=> __('Replace previously imported Content', 'ntfs'),
                'can_skip'		=> true,
                'can_replace'	=> true
            ),
        );
        ...
        $import_products_step = array(
            'products' => array(
                'id' => 'products',
                'title' => __('Import Products', 'ntfs'),
                'icon' => 'welcome-content-menus',
                'view' => 'getStepProducts',
                'callback_products_import' => 'import_products',
                'callback_products_replace' => 'replace_products',
                'button_products_import_text' => __('Import Products', 'ntfs'),
                'button_products_replace_text' => __('Replace previously imported Products', 'ntfs'),
                'can_skip' => true,
                'can_replace' => true
            ),
        );
        ...
        $install_plugin_step = array(
            'plugins' => array(
                'id'			=> 'plugins',
                'title'			=> __('Install Theme Plugins', 'ntfs'),
                'icon'			=> 'admin-plugins',
                'view'			=> 'get_step_plugins',
                'callback'		=> 'install_plugins',
                'button_text'	=> __('Install Plugins', 'ntfs'),
                'can_skip'		=> false
            ),
        );
    ...
    public function get_step_plugins() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Install plugins included with the Theme. <br>Click the "Install Plugins" button to start the installation.', 'ntfs')
        );
    ...
    public function get_step_content() {
        ...
        if($hideImport) {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Content has already been imported. Please skip this step', 'ntfs')
           );
        } else {
            $content['summary'] = sprintf(
                '<p>%s</p>',
                __('Theme has Pages, Images, Menu, Header, and Footer. </br></br>Do you want to import the Content?', 'ntfs')
            );
        }
        $content['import_options'] = sprintf(
            ...
            __('Import Sidebars Content', 'ntfs')
        );
        if (class_exists('Woocommerce')) {
            $content['import_options'] .= sprintf(
                ...
                __('Import Products Content to Woocommerce', 'ntfs')
            );
        }
    ...
    public function getStepProducts() {
        ...
        if (file_exists($contentPath)) {
            ...
            if (!empty($data['Parameters']['productsJson'])) {
                ...
                if (!empty($productsData['products'])) {
                    $options .= '<option value="content-source" ' . selected(get_option('np_products_source'), 'content-source', false) . '>' . __('Content', 'ntfs') . '</option>';
                }
            }
        }
        ...
        if (file_exists($themeProductsPath)) {
            ...
            $data = json_decode($jsonContent, true);
            if (!empty($data['products'])) {
                $options .= '<option value="theme-source" ' . selected(get_option('np_products_source'), 'theme-source', false) . '>' . __('Theme', 'ntfs') . '</option>';
            }
        }

        $content['detail'] = '
    <div class="products-options">
        <label for="np_products_source">' . __('We have found Products that you can import to your site. </br></br>Do you want to continue with importing?', 'ntfs') . '</label>
        </br></br>
    ...
    <p>';
        ...
        if (isset($old_products_data['products']) && count($old_products_data['products']) > 0) {
            $content['detail'] .= sprintf(
                '<p><b>%s</b></p>',
                __('Please note that some Products already exist in your database, and the import will overwrite the current ones.', 'ntfs')
            );
        }
        return $content;
    }
    ...
    <div class="theme-appearance-options">
        <label for="np_theme_appearance">' . __('Choose the source for your site\'s Colors, Fonts, Header, and Footer, as well as for the Blog, E-Commerce, and other Templates.', 'ntfs') . '</label>
        </br></br>
        <select name="np_theme_appearance" id="np_theme_appearance">
            <option value="theme-option" ' . selected(get_option('np_theme_appearance'), 'theme-option', false) . '>' . __('Theme', 'ntfs') . '</option>
            <option value="plugin-option" ' . selected(get_option('np_theme_appearance'), 'plugin-option', false) . '>' . __('Plugin', 'ntfs') . '</option>
        </select>
    </div>
    <p>' . sprintf(
                __('You can change it later in the <a href="%s">Nicepage Settings</a>', 'ntfs'),
    ...
    public function get_step_done() {
        ...
        $content['summary'] = sprintf(
            '<p>%s</p>',
            __('Congratulations! The theme has been activated and your website is ready.', 'ntfs')
        );
        $content['summary'] .= sprintf( '<p>%s</p>', 'Create a new page with the Nicepage Editor.', 'ntfs' );
	...
	public function setup_plugins() {
		if (! check_ajax_referer('npwizard_nonce', 'wpnonce') || empty($_POST['slug'])) {
			wp_send_json_error(array('error' => 1, 'message' => esc_html__('No Slug Found', 'ntfs')));
		}
		$json = array();
		...
		foreach ($plugins['activate'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Activating Plugin', 'ntfs'),
		...
		foreach ($plugins['update'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Updating Plugin', 'ntfs'),
				);
				break;
			}
		}
		foreach ($plugins['install'] as $slug => $plugin) {
			if ($_POST['slug'] == $slug) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__('Installing Plugin', 'ntfs'),
				);
				break;
			}
		}
		if ($json) {
			...
			wp_send_json(array('done' => 1, 'message' => esc_html__('Success', 'ntfs')));
		}
		exit;
	}
    ...
    public function setup_content() {
        ...
        $json = array(
            ...
            'message'       => esc_html__('Import Content', 'ntfs'),
       );
        if ($json) {
            $json['hash'] = md5(serialize($json)); // used for checking if duplicates happen, move to next plugin
            wp_send_json($json);
