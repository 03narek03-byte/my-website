...
function theme_breadcrumbs_items($args) {
    ...
    if (!is_front_page()) {
        $items[] = theme_breadcrumbs_link($args, get_home_url(), '', __('Home', 'ntfs'));
    }
    ...
    if (is_tag()) {
        $items[] = theme_breadcrumbs_text($args, __("Tag: ", 'ntfs') . single_tag_title('', FALSE));
    }
    if (is_404()) {
        $items[] = theme_breadcrumbs_text($args, __("404 - Page not Found", 'ntfs'));
    }
    if (is_search()) {
        $items[] = theme_breadcrumbs_text($args, __("Search", 'ntfs'));
    }
    if (is_year()) {
        $items[] = theme_breadcrumbs_text($args, get_the_time('Y'));
    }
    if (is_author()) {
        $items[] = theme_breadcrumbs_text($args, sprintf(esc_attr(__('View all posts by %s', 'ntfs')), get_the_author()));
    }
